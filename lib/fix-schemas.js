...
exports.fixGeneratedSchemas = fixGeneratedSchemas;
...
async function maybeWrite(p, content, original) {
    if (content !== original) {
        await fs_1.promises.writeFile(p, content, 'utf-8');
    }
}
 ...
 */
function isPlainArgsFile(basename) {
    // Must end in Args.schema.ts
    if (!basename.endsWith('Args.schema.ts'))
        return false;
    // Must NOT contain an operation verb before "Args"
    const name = basename.replace(/\.schema\.ts$/, '');
    return !/(?:Create|Update|Delete|Upsert|FindFirst|FindMany|FindUnique|Aggregate|GroupBy)Args$/.test(name);
}
async function fixArgsSchemas(objectsDir, allFiles) {
    // Collect plain Args files
    const argsFiles = allFiles.filter((f) => isPlainArgsFile(path_1.default.basename(f)));
    if (argsFiles.length === 0)
        return;
    // Map from old schema-name to new schema-name for importer patching
    const renames = [];
    for (const oldPath of argsFiles) {
        const oldBase = path_1.default.basename(oldPath, '.schema.ts'); // e.g. "UserArgs"
    ...
    const schemasDir = path_1.default.dirname(objectsDir);
    const remaining = [
        ...(await collectSchemaFiles(schemasDir)),
        path_1.default.join(schemasDir, 'index.ts'),
    ];
    for (const file of remaining) {
        ...
        for (const { oldBase, newBase, oldConst, newConst } of renames) {
            ...
            content = content
                .replace(new RegExp(`(['"])((?:\\.\\./|\\./)(?:[^'"]*/)?)${oldBase}\\.schema\\1`, 'g'), `$1$2${newBase}.schema$1`)
...
function classifyFilter(schemaName) {
    if (!/Filter/.test(schemaName))
        return 'none';
    // Order matters: BigInt before Int
    if (/DateTime/.test(schemaName))
        return 'datetime';
    if (/BigInt/.test(schemaName))
        return 'bigint';
    if (/Decimal/.test(schemaName))
        return 'int_decimal';
    if (/Float/.test(schemaName))
        return 'float';
    if (/Int/.test(schemaName))
        return 'int_decimal';
    if (/String/.test(schemaName))
        return 'string';
    return 'none';
}
/**
 ...
 */
function fixInNotIn_withString(content, scalar) {
    return content.replace(new RegExp(`z\\.union\\(\\[z\\.${scalar}\\(\\)\\.array\\(\\), z\\.${scalar}\\(\\)\\]\\)\\.optional\\(\\)`, 'g'), `z.union([z.${scalar}().array(), z.string().array()]).optional()`);
}
/**
 ...
 */
function fixInNotIn_arrayOnly(content, scalar) {
    return content.replace(new RegExp(`z\\.union\\(\\[z\\.${scalar}\\(\\)\\.array\\(\\), z\\.${scalar}\\(\\)\\]\\)\\.optional\\(\\)`, 'g'), `z.${scalar}().array().optional()`);
}
/**
 ...
 */
function fixScalarOps(content, scalar) {
    return content.replace(new RegExp(`z\\.${scalar}\\(\\)\\.optional\\(\\)`, 'g'), `z.union([z.${scalar}(), z.string()]).optional()`);
}
/**
 ...
 *     ])
 */
function fixNotField(content, scalar) {
    // Single-line format: .union([z.scalar(), z.lazy(() =>
    // Note: no `z.` prefix before `.union` — it is a chained method call.
    content = content.replace(new RegExp(`\\.union\\(\\[z\\.${scalar}\\(\\), z\\.lazy\\(\\(\\) =>`, 'g'), `.union([z.${scalar}(), z.string(), z.lazy(() =>`);
 ...
 */
function fixZodTypeGeneric(content) {
    ...
    return content.replace(/z\.ZodType<Prisma\.(\w+Filter\w*)>/g, (_, name) => name.includes('<') ? `z.ZodType<Prisma.${name}>` : `z.ZodType<Prisma.${name}<never>>`);
}
async function fixFilterSchema(filePath) {
    const schemaName = path_1.default.basename(filePath, '.schema.ts');
...
// Public entry point
// ---------------------------------------------------------------------------
async function fixGeneratedSchemas(outputDir) {
    const objectsDir = path_1.default.join(outputDir, 'schemas', 'objects');
...
//# sourceMappingURL=fix-schemas.js.map